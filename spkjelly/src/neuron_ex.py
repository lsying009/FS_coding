...
class AdLIFNode(neuron.BaseNode):
    ...
    def multi_step_forward(self, x_seq: torch.Tensor):
        ...
        if self.backend == 'torch':
            ...
            # x_seq, v_init, I_init, v_th, v_reset, mem_decay, syn_decay, forward_kernel, backward_kernel
            spike_seq, v_seq, h_seq = AdLIFNodeATGF.apply(x_seq.flatten(1), self.v.flatten(0), self.w.flatten(0),
                                                                    self.v_threshold, self.v_reset, 
                                                                    self.decay_mem.sigmoid().flatten(0).to(x_seq),
                                                                    self.decay_osc.sigmoid().flatten(0).to(x_seq), 
                                                                    self.a.flatten(0).to(x_seq), self.b.flatten(0).to(x_seq), ######
                                                                    self.decay_mem.numel(),
                                                                    self.decay_mode, 
                                                                    self.forward_kernel,
                                                                    self.backward_kernel)
            spike_seq = spike_seq.reshape(x_seq.shape)
