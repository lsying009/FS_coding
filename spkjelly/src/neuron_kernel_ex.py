...
class CuAdLIFNodeFPTTKernel(CuNeuronFPTTKernel):
    def __init__(self, decay_mode, hard_reset: bool, dtype: str):
        super().__init__(hard_reset, dtype)
        self.decay_mode = decay_mode
        self.add_param(ctype=f'const {dtype} *', cname='mem_decay')
    ...
    def neuronal_charge(self) -> str:
        ...
        # codes += cfunction.add(z='h_seq[t]', x='LIFNodeFPTTKernel_temp_var', y='I_I_seq[t]', dtype=self.dtype)
        codes += cfunction.add(z='LIFNodeFPTTKernel_temp_var', x='LIFNodeFPTTKernel_temp_var', y='I_I_seq[t]', dtype=self.dtype)
...
class CuAdLIFNodeBPTTKernel(CuNeuronBPTTKernel):
    def __init__(self, surrogate_function: Callable, decay_mode:str, hard_reset: bool, detach_reset: bool, dtype: str):
        super().__init__(surrogate_function, hard_reset, detach_reset, dtype)
        self.decay_mode = decay_mode
        
        # self.add_param(ctype=f'{dtype} *', cname='grad_w_seq')
    ...
    def core(self):
        ...
        core_codes.append(cfunction.mul(z='grad_w', x='grad_w', y='grad_w_next_to_w', dtype=self.dtype))
...
class CuAdLIFNodeATGF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_seq: torch.Tensor, v_init: torch.Tensor, I_init: torch.Tensor, w_init: torch.Tensor, v_th: float, v_reset: float or None,
                mem_decay: torch.Tensor, syn_decay: torch.Tensor, osc_decay: torch.Tensor, a: torch.Tensor, b: torch.Tensor, 
                output_size: int, decay_mode: str,
                forward_kernel: CuAdLIFNodeFPTTKernel, backward_kernel: CuAdLIFNodeBPTTKernel):
        if x_seq.dtype == torch.float16 and v_init.numel() % 2 != 0:
            raise ValueError('When using the the PLIF neuron with half2 cupy backend, the numer of neurons should be even to avoid the wrong gradient of tau caused by padding!')
        ...
        # py_dict['noise_seq'] = 0.001*py_dict['x_seq'].detach().abs().mean(-1, keepdim=True)*torch.randn_like(py_dict['x_seq'], requires_grad=False)
        ...
        CuNeuronATGFBase.ctx_save(ctx, requires_grad, py_dict['s_s_seq'], py_dict['h_h_seq'], py_dict['v_v_seq'], py_dict['I_I_seq'], py_dict['w_w_seq'], blocks=blocks, threads=threads,
                        numel=py_dict['numel'], N=py_dict['N'], v_th=py_dict['v_th'], v_reset=py_dict['v_reset'],
                        backward_kernel=backward_kernel, mem_decay=py_dict['mem_decay'], syn_decay=py_dict['syn_decay'], 
                        osc_decay=py_dict['osc_decay'], a=py_dict['a'], b=py_dict['b'],
                        output_size=output_size, decay_mode=decay_mode)
        
        return py_dict['s_s_seq'][1:,], py_dict['v_v_seq'][1:, ], py_dict['I_I_seq'][1:, ],  py_dict['h_h_seq'][1:,]
    ...
    def backward(ctx, grad_spike_seq: torch.Tensor, grad_v_seq: torch.Tensor, grad_I_seq: torch.Tensor, grad_h_seq: torch.Tensor):
        ...
        py_dict['s_s_seq'] = ctx.saved_tensors[0]
        py_dict['h_h_seq'] = ctx.saved_tensors[1]
        py_dict['v_v_seq'] = ctx.saved_tensors[2]
        py_dict['I_I_seq'] = ctx.saved_tensors[3]
        ...
        return py_dict['grad_x_seq'], py_dict['grad_v_init'], py_dict['grad_I_init'], py_dict['grad_w_init'], None, None, py_dict['grad_mem_decay'], py_dict['grad_syn_decay'], py_dict['grad_osc_decay'], py_dict['grad_a'], py_dict['grad_b'], None, None, None, None
...
class AdLIFNodeBPTTKernel(NeuronBPTTKernel):
    ...
    def grad_h_next_to_x(self) -> str:
        if self.decay_mode == 's':
            return cfunction.sub(z=f'const {self.dtype} grad_h_next_to_x', x=cfunction.constant(None, x=1., dtype=self.dtype), y='mem_decay[0]', dtype=self.dtype)
        else:
            return cfunction.sub(z=f'const {self.dtype} grad_h_next_to_x', x=cfunction.constant(None, x=1., dtype=self.dtype), y='mem_decay[index]', dtype=self.dtype)
...
class AdLIFNodeATGF(torch.autograd.Function):
    ...
    def forward(ctx, x_seq: torch.Tensor, v_init: torch.Tensor, w_init: torch.Tensor, v_th: float, v_reset: float or None,
        ...
        NeuronATGFBase.ctx_save(ctx, requires_grad, py_dict['s_s_seq'], py_dict['h_h_seq'], py_dict['v_v_seq'], py_dict['w_w_seq'], blocks=blocks, threads=threads,
                        numel=py_dict['numel'], N=py_dict['N'], v_th=py_dict['v_th'], v_reset=py_dict['v_reset'],
    ...
    def backward(ctx, grad_spike_seq: torch.Tensor, grad_v_seq: torch.Tensor, grad_h_seq: torch.Tensor):
        ...
        py_dict['h_h_seq'] = ctx.saved_tensors[1]
        py_dict['v_v_seq'] = ctx.saved_tensors[2]
        py_dict['w_w_seq'] = ctx.saved_tensors[3]
...
# culif_fptt_kernel = CuAdLIFNodeFPTTKernel(hard_reset=True, decay_mode='m', dtype='float')
# print(culif_fptt_kernel.full_codes)
# for key, value in culif_fptt_kernel.cparams.items():
#     print(f'key="{key}",'.ljust(20), f'value="{value}"'.ljust(20))

# from .surrogate_ex import MySoftSign
# kernel = ParametricCuLIFNodeBPTTKernel(surrogate_function=MySoftSign().cuda_codes, decay_mode='s', hard_reset=True, detach_reset=True, dtype='float')
# print(kernel.full_codes)
# for key, value in kernel.cparams.items():
#     print(f'key="{key}",'.ljust(22), f'value="{value}"'.ljust(20))

# kernel = ParametricLIFNodeBPTTKernel(surrogate_function=MySoftSign().cuda_codes, hard_reset=True, detach_reset=True, decay_input=True, dtype='float')
# print(kernel.full_codes)

# from surrogate_ex import MySoftSign
# kernel = CuAdLIFNodeBPTTKernel(surrogate_function=MySoftSign().cuda_codes, decay_mode='s', hard_reset=True, detach_reset=True, dtype='float')
# print(kernel.full_codes)
# for key, value in kernel.cparams.items():
#     print(f'key="{key}",'.ljust(22), f'value="{value}"'.ljust(20))
